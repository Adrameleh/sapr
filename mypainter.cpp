...
void MyPainter::paintEvent(QPaintEvent *)
{
    ...
    QPainter paint(this);        // Инициализация

    {
        ...
        QPointF pointRodsLT;
        QPointF pointRodsRB;
        QPointF pointLongRodsLT;
        QPointF pointLongRodsRB;
        QPointF pointTopSpaceNearRodsLT;
        ...
        pointRodsRB.setY(((((pointWorkRightBot.y() - pointWorkLeftTop.y())/3) * 2) + pointWorkLeftTop.y()));
...
void MyPainter::mouseMoveEvent(QMouseEvent *event)
{
    ...
    for (int i = 0; i < vecRods.size(); i++)
    {
        if ( vecRods.at(i)->pointLT.x() < event->x()
             ...
             && rodAlreadyFound == false)
        {
            cursorOn = i;
            rodAlreadyFound = true;
        }
        else
        {
            if (!rodAlreadyFound)
